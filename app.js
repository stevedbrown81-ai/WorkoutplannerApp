...
function renderWorkout(workout) {
  ...
  elements.workoutTitle.textContent = workout.circuitName || workout.typeLabel;
  elements.workoutSubtitle.textContent = workout.circuitId
    ? `${title(workout.readiness)} readiness · ${workout.circuitEquipment === "double" ? "Double KB" : "Single KB"} · ${workout.circuitCategory}`
    : `${title(workout.readiness)} readiness · ${workout.format === "circuit" ? "2-round pump circuit" : "2 working sets"}`;
  ...
  elements.exerciseList.innerHTML = workout.exercises.length ? workout.exercises.map((item) => {
    ...
    return `${group}<article class="exercise-card">
      ...
      ${item.swappable === false ? "" : `<button type="button" class="swap-button" data-swap="${item.id}" aria-label="Swap ${escapeHtml(item.name)}">Swap</button>`}
    </article>`;
...
function regenerateWorkout() {
  ...
  state.workout = generateWorkout(workoutInput());
  renderWorkout(state.workout);
  elements.workoutStatus.textContent = state.workout.circuitId ? "A different complete circuit is ready." : "A coordinated alternative is ready.";
}
...
async function saveCompletionFromDialog() {
  ...
  const entry = {
    ...
    circuitId: state.workout.circuitId || "",
    circuitName: state.workout.circuitName || "",
...
function renderWeeklyPlan(plan) {
  ...
  elements.plannedDays.innerHTML = plan.days.map((day) => {
    ...
    return `<details class="planned-day"><summary><span>${day.day.slice(0, 3)}</span><div><h3>${day.day} · ${escapeHtml(day.workout.circuitName || day.workout.typeLabel)}</h3><p>${escapeHtml(day.workout.locationLabel)} · ${day.workout.duration} min</p></div><i></i></summary>
...
function renderHistory() {
  ...
  elements.historyList.innerHTML = state.history.map((entry) => {
    ...
    return `<article class="history-item"><time datetime="${entry.completedAt}"><strong>${date.getDate()}</strong><span>${date.toLocaleDateString("en-GB", { month: "short" })}</span></time><div><p>${escapeHtml(entry.locationLabel || entry.location)} · ${entry.duration} min</p><h2>${escapeHtml(entry.circuitName || title(entry.actualType))}</h2><span>${escapeHtml(names)}</span></div></article>`;
  }).join("");
}
