...
export function defaultWeek() {
  ...
  return DAYS.map((day, index) => ({ day, session: sessions[index], location: index === 3 ? "home" : "kongs", duration: index === 3 ? 30 : 40, hotelEquipment: [] }));
}
...
export function generateWeeklyPlan(days, options = {}) {
  ...
  const plannedDays = days.map((day, index) => {
    ...
    coordinatedHistory.unshift({
      actualType: workout.type,
      actualExercises: workout.exercises.map(({ id }) => id),
      circuitId: workout.circuitId || "",
      circuitName: workout.circuitName || "",
    });
    return { ...day, workout };
  });
